...
const loadCart = async (req, res) => {
    try {
        ...
        if (!User) {
            ...
            // const cartDetails = await Cart.findOne({ userId: User._id }).populate({ path: 'items.product_id' }).populate({path:"items.offer"});
            const cartDetails = await Cart.findOne({ userId: User._id })
    .populate({
        ...
        populate: {
            ...
            let discountedAmount = 0; // Initialize discounted amount
            ...
            if (cartDetails) {
                cartDetails.items.forEach((cartItem) => {
                    ...
                    // Calculate discounted amount if an offer is available
                    if (cartItem.product_id.offer) {
                        let discountedPrice = cartItem.product_id.price - (cartItem.product_id.price * cartItem.product_id.offer.percentage / 100);
                        discountedAmount = discountedPrice * cartItem.quantity;
                    }
                });
            }
            console.log(cartDetails,"kkkkkkkkkkkkkkkkkkkkkkkjjjjjjjjjjjjjjjjj");

            res.render('userSide/cart', { user, cartDetails, subTotal: originalAmount, discountedAmount });
        }
    } catch (error) {
