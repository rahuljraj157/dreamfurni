...
//     const products = await Product.findById(productId).populate({ path: 'category' }).populate("offer");
//     console.log(products,"hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
...
const loadProductPage = async (req, res) => {
  try {
    ...
    const user = await usersDb.findOne({ _id: userId });
    const productId = req.params.id;
    console.log(productId,"sssssssssssssssssssssssss")
    ...
    console.log(products,"yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy")

    
    let discountedPrice = products.price;
    if (products.offer!==null) {
      const discountPercentage = products.offer.percentage;
      const discountAmount = (discountPercentage / 100) * products.price;
      discountedPrice = products.price - discountAmount;
    }

    
    res.render('userSide/productPage', { userId, user, products, discountedPrice });
